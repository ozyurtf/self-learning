...
class Truck:
    def __init__(self, lesson, truck_speed, env_x_range, env_y_range, train_x_cab_range, train_y_cab_range_abs, train_configs, test_config, gif = False, display = False):
        self.W = 1 
        self.L = 1 * self.W 
        self.d = 4 * self.L 
        self.s = truck_speed
        ...
        self.train_x_cab_range = train_x_cab_range 
        self.train_y_cab_range_abs = train_y_cab_range_abs
        ...
        if self.display:
            ...
            self.patches = list()
            self.ax.axis('equal')
            self.ax.axis([self.box[0], self.box[1], self.box[2], self.box[3]])
            self.ax.set_xticks([]); self.ax.set_yticks([])
            self.ax.axhline(); self.ax.axvline()
            
            self.red_x0 = train_x_cab_range[0]
            self.red_y0 = -train_y_cab_range_abs[1]
            self.red_width = train_x_cab_range[1] - self.red_x0
            self.red_height = train_y_cab_range_abs[1] * 2            
            
            rectangle_red = patches.Rectangle((self.red_x0, self.red_y0), self.red_width, self.red_height,
                                              edgecolor = "darkred",     
                                              facecolor = "none",  
                                              alpha=1,                 
                                              linewidth=3)
            ...
            plt.text(self.red_x0 + self.red_width - 0.1, 
                     self.red_y0 + self.red_height - 0.1,
                     'Training Region',
                     color='white',
                     ha='right',
                     va='top',
    ...
    def reset(self, ϕ = 0, train_test = "train", test_seed = 1):
        ...
        if train_test == "train":
            config = self.train_configs.get(self.lesson)
        else: 
    ...
    def _draw_trajectories(self, test_seed): 
        ...
        rectangle_green = patches.Rectangle((self.box[0], self.box[2]), self.box[1] - self.box[0], self.box[2] - self.box[1],
                                            facecolor='green',           
                                            edgecolor='darkgreen',       
                                            alpha=0.3,                 
                                            linewidth=2)        
...
def train_emulator(truck,
    ...
    for episode in tqdm(range(episodes)):
        truck.reset(ϕ = 0, train_test = "test")
        while truck.valid():
...
def train_controller(truck,
    ...
    for i in tqdm(range(epochs)):
        random_deg = generate_random_deg()
        ϕ = deg2rad(random_deg) 
        truck.reset(ϕ = ϕ, train_test = "test")
        x, y, θ0, _, _, θ1 = truck.state()
        ϕ = truck.ϕ
...
def main(args):
    ...
    gif = args.gif=="True"  
    lesson = 0   
    train_configs = create_train_configs(x_cab_range = train_x_cab_range, 
                                         y_cab_range_abs = train_y_cab_range_abs, 
                                         cab_angle_range_abs = train_cab_angle_range_abs,
                                         cab_trailer_angle_diff_range_abs = train_cab_trailer_angle_diff_range_abs, 
                                         num_lessons = train_num_lessons)
    ...
    if train_emulator_flag:
        ...
        for lesson in range(1, train_num_lessons + 1):
            ...
            truck_emulator = Truck(lesson = lesson, 
                                   truck_speed = truck_speed, 
                                   env_x_range = env_x_range, 
                                   env_y_range = env_y_range, 
                                   train_x_cab_range = train_x_cab_range, 
                                   train_y_cab_range_abs = train_y_cab_range_abs, 
                                   train_configs = train_configs, 
                                   test_config = test_config, 
                                   gif = False,
                                   display = False)            
            emulator = train_emulator(truck = truck_emulator, 
                                      lesson = lesson,
                                      emulator = emulator,
                                      episodes = 20_000,
    ...
    if train_controller_flag:        
        ...
        for lesson in range(1, train_num_lessons + 1): 
            print(" Lesson {}:".format(lesson))
            truck_controller = Truck(lesson = lesson, 
                                     truck_speed = truck_speed, 
                                     env_x_range = env_x_range, 
                                     env_y_range = env_y_range, 
                                     train_x_cab_range = train_x_cab_range, 
            ...
            controller = train_controller(truck = truck_controller, 
                                          lesson = lesson, 
    ...
    truck_test = Truck(lesson = test_lesson, 
                       truck_speed = truck_speed, 
                       env_x_range = env_x_range, 
                       env_y_range = env_y_range, 
                       train_x_cab_range = train_x_cab_range, 
    ...
    for test_seed in range(1, num_test_trajectories + 1):
        with torch.no_grad():
            random_deg = generate_random_deg()
            ϕ = deg2rad(random_deg) 
            truck_test.reset(ϕ = ϕ, train_test = "test", test_seed = test_seed)    
            ϕ = torch.tensor([truck_test.ϕ], dtype=torch.float32)
            step = 0
            while truck_test.valid():
                x, y, θ0, _, _, θ1 = truck_test.state()
                state = torch.tensor([x, y, θ0, θ1], dtype = torch.float32) 
                ϕ_state = torch.cat((ϕ, state))
                next_ϕ = test_controller(ϕ_state) 
                truck_test.step(ϕ.item())
                truck_test.draw()
                ϕ = next_ϕ
                step += 1
            truck_test._draw_trajectories(test_seed)
            x, y, θ0, trailer_x, trailer_y, θ1 = truck_test.state()  
            num_jackknifes += truck_test.is_jackknifed()
            print(f"Number of Steps: {step}")
            print(f"Is Jackknifed ? {truck_test.is_jackknifed()}")
            print(f"Trailer x: {trailer_x:.3f}, Trailer y: {trailer_y:.3f}")
            print()
    print(f"Number of Jackknifes: {num_jackknifes}")
    if truck_test.gif:
        truck_test.generate_gif()
